...
| [last-20-percent](skills/last-20-percent) | Finds and finishes the experiential 20% agents skip — magic-moment scene, scene-list-minus-noun-list decomposition, golden artifacts before generators, a front-loaded finish list treated like failing tests, and a cold end-user walk before calling anything done |
...
| [principal-engineering-review](skills/principal-engineering-review) | Production-grade engineering review and implementation — verify-the-repo-first discovery, smallest-safe-slice changes, frontend/backend/security/migration/CI-CD standards, Expand-Migrate-Contract schema changes, ADR and trade-off templates, P0–P3 review output, and a completion bar that requires evidence |
...
| [resource-aware-test-runs](skills/resource-aware-test-runs) | Keep local test suites from wrecking an interactive Windows desktop without weakening verification — narrow red loops, Vitest worker caps (both --minWorkers and --maxWorkers), full gates at the boundary, Cargo package-cache/artifact-lock contention handling, and an explicit statement of what was not verified |
| [running-paid-api-jobs](skills/running-paid-api-jobs) | Treat an authorized paid API/model/eval run as a budget-bound operation — lock the exact command and cap, preflight without spend, require checkpoint/resume before multi-hour jobs, run once under capture, then report realized cost vs cap, cache metrics, and secret-scanned artifacts |
